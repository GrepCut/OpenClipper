...
function decide(ctx: ArbiterSampleContext, params: Partial<ArbiterParams> = {}) {
  return decideLayoutStrategy(ctx, { ...LEGACY_ARBITER_PARAMS, ...params });
}
...
describe("layout arbiter", () => {
  ...
  it("allows split and contain under production arbiter params", () => {
    expect(decide(context({ desiredMode: "split", controllerReasonCodes: ["x"] }), DEFAULT_ARBITER_PARAMS).selectSemantic).toBe(true);
    expect(decide(context({ desiredMode: "contain", controllerReasonCodes: ["x"] }), DEFAULT_ARBITER_PARAMS).selectSemantic).toBe(true);
  });
