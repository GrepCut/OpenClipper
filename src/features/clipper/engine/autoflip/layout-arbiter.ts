...
/**
 * Production policy (`analyze-subjects.ts` always builds with `iteration10:
 * true`, which is what selects this constant). The visibility controller is
 * always enabled in production and reports its decision via
 * `controllerReasonCodes` — the arbiter just needs to allow split/contain
 * modes through so that decision isn't overridden.
 */
