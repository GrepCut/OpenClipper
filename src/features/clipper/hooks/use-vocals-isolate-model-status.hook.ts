...
import type { VocalsIsolateModelStatus } from "../../../services/types/transcription.types";
...
export function useVocalsIsolateModelStatus() {
  const [modelStatus, setModelStatus] = useState<VocalsIsolateModelStatus | null>(null);
  ...
  const refreshStatus = useCallback(async (): Promise<VocalsIsolateModelStatus | null> => {
    if (!isTauri()) return null;
    try {
      const status = await transcriptionService.getVocalsIsolateModelStatus();
  ...
  const handleDownload = useCallback(async (): Promise<VocalsIsolateModelStatus | null> => {
    setError(null);
    ...
    setDownloadTotal(null);
    try {
      await transcriptionService.downloadVocalsIsolateModel();
  ...
  const handleDelete = useCallback(async () => {
    setError(null);
    try {
      await transcriptionService.deleteVocalsIsolateModel();
      await refreshStatus();
    } catch (deleteError) {
      if (isMountedRef.current) {
