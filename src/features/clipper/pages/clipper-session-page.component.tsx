...
function ClipperSessionContent({
  ...
  if (loader.phase === "error") {
    return (
      <ClipperLayout>
        <Box p={8} borderRadius="2xl" maxW="560px" mx="auto" {...errorPanel}>
...
export function ClipperSessionPage() {
  ...
  useEffect(() => {
    if (!projectId) {
      navigateRef.current("/clipper", { replace: true });
      return;
    }

    // Sub-route changes (/render, /rendering, /exports) must not remount the session.
