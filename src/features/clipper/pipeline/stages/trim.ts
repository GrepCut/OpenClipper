...
import {
  ...
  getNativeFilePath,
  isNativeBacked,
} from "../../platform/native-source";
import { clipperLog, clipperWarn } from "../../shared/logger";
import {
...
/** Trims a sub-segment from the already-trimmed range file (for per-clip export). */
export async function trimSubClipFromRange(
  session: ClipperSession,
  startSec: number,
  endSec: number,
  reporter: PipelineReporter,
  options: { signal: AbortSignal; reportStage?: boolean; clipIndex?: number },
): Promise<TrimStageResult> {
  const rangeFile = session.rangeTrimmedFile ?? session.trimmedFile;
  if (!rangeFile) {
    throw new Error("Range video is not ready — cannot trim sub-clip.");
  }

  const reportStage = options.reportStage ?? true;
  const progressKey =
    options.clipIndex != null ? `${options.clipIndex}:trim` : "trim";

  if (reportStage) {
    reporter.stage("uploading", "Preparing clip segment…");
    reporter.stageProgress(0);
  }

  const trimmedBuffer = await extractSegmentBuffer(rangeFile, startSec, endSec, {
    signal: options.signal,
    onProgress: (ratio) => {
      if (reportStage) {
        reporter.stageProgress(ratio);
      } else {
        reporter.renderProgress(progressKey, ratio);
      }
    },
  });
  const trimmedFile = new File([trimmedBuffer], `clip-${startSec}-${endSec}.mp4`, { type: "video/mp4" });
  const trimmedVideoUrl = URL.createObjectURL(trimmedFile);
  if (reportStage) {
    reporter.stageProgress(1);
  } else {
