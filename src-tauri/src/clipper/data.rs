...
use crate::video::ffmpeg::studio_clip::{
    extract_studio_clip_blocking, ExtractClipperStudioClipResult,
};
use crate::video::ffmpeg::studio_thumbnails::{
    extract_studio_thumbnails_blocking, ExtractClipperStudioThumbnailsResult,
};
...
pub(crate) async fn extract_studio_clip_for_project(
    app: &AppHandle,
    project_id: &str,
    start_sec: f64,
    end_sec: f64,
    ...
    let dir = clipper_project_data_dir(app, project_id)?;
    tokio::task::spawn_blocking(move || extract_studio_clip_blocking(&dir, start_sec, end_sec))
        .await
        .map_err(|e| format!("Task join error: {e}"))?
}
